...
- **El sistema decide en Triage que no es una vulneracion.** El caso pasa a Descartado (estado terminal, reabribile con justificacion); ambos cronometros se detienen y el motivo queda en el historial, visible para Auditor.
...
1. **Archivo:** `03_modulos/MOD-024_ficha.md` (seccion G, regla 1) frente a `02_validacion/mapa_modulos.json`. Que dice cada fuente: `mapa_modulos.json` declara en el `alimenta_a` de MOD-024 solo cinco receptores del evento "cambio de bandera" (MOD-002, MOD-007, MOD-011, MOD-015 y MOD-017); la propia ficha de MOD-024 (que es la ficha del modulo propietario de la bandera) agrega un sexto receptor, MOD-008, porque la ficha ya redactada de MOD-008 (seccion G, regla 3) declara una automatizacion propia disparada exactamente por ese evento. Version adoptada en este documento (paso 3 y 7 del ejemplo principal del Caso 11): se incluye a MOD-008 entre los receptores del evento, siguiendo a la ficha del modulo propietario de la bandera (MOD-024) sobre el campo `alimenta_a` de `mapa_modulos.json`, que queda senalado en ambas fichas como pendiente de actualizar en una proxima revision del mapa.
