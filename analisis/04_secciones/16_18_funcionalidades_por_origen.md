...
| Registro de consentimiento reforzado (sensible/biometrico con firma) | MOD-007 (Consentimiento) | OBL-CONS-04 | Ley para la Proteccion de Datos Personales (D.L. 144), Art. 26 inc. 4 | OBLIGATORIO | - | MVP |
...
| Consulta publica del Aviso de Privacidad vigente (cobertura MVP ya satisfecha por el formulario interno seguro de MOD-011); ampliada por el Portal del Titular (MOD-012, V1) | MOD-012 (Portal del Titular) | OBL-DOC-04 | Ley para la Proteccion de Datos Personales (D.L. 144), Art. 61 inc. 2 | OBLIGATORIO | - | MVP/V1 |
...
| Gestor de Aviso de Privacidad con checklist de 9 literales (Art. 24) y 5 elementos (Art. 7); Formulario interno seguro con los 7 elementos del Art. 18 (cobertura MVP); ampliado por el Portal del Titular (MOD-012, V1) | MOD-012 (Portal del Titular); MOD-008 (Documentos y Politicas); MOD-011 (ARCO-POL) | OBL-PLAZO-04 | Ley para la Proteccion de Datos Personales (D.L. 144), Art. 61 inc. 2 | OBLIGATORIO | - | MVP/V1 |
...
1. **OBL-TRANSF-06: la seccion Q de `03_modulos/MOD-010_ficha.md` la asocia con la fila equivocada.** La fila "Solicitud de opinion previa a la ACE" de la seccion Q de MOD-010 cita en su justificacion "RECOMENDADO, OBL-TRANSF-06", como si esa obligacion fuera la facultad opcional de pedir opinion previa a la ACE sobre un flujo transfronterizo (Art. 45). Sin embargo, `01_legal/matriz_obligaciones.json` define OBL-TRANSF-06 como "Carga de la prueba en transferencias internacionales" (Art. 54 inc. 2), clasificada OBLIGATORIO, sin ninguna relacion textual con la opinion previa del Art. 45. Se adopto la definicion de `matriz_obligaciones.json` por jerarquia (fuente legal primaria y matriz sobre ficha de modulo): OBL-TRANSF-06 se mapeo en la seccion 16 a la fila "Reportes y paquete de evidencia exportable con hash", cuya propia justificacion en la misma ficha ("es lo que hace probatorio al modulo... sin exportacion verificable, el registro no sirve ante una auditoria") si corresponde a la carga de la prueba. La "Solicitud de opinion previa a la ACE" se reclasifico en la seccion 18.2 de este documento como automatizacion opcional, sin OBL-ID propio, que es coherente con el Art. 45 LPDP (la puesta en conocimiento es obligatoria pero la opinion previa es una facultad).
2. **OBL-AUD-02 aparece en dos fichas distintas con el mismo contenido.** `03_modulos/MOD-018_ficha.md` (Auditoria de Cumplimiento) incluye en su propia seccion Q la fila "Integracion con un futuro mecanismo de certificacion de la ACE (OBL-AUD-02)", pese a que la tabla de cobertura de `06_mapa_definitivo_de_modulos.md` seccion 8 asigna OBL-AUD-02 como propietario a MOD-024 (Centro Regulatorio) y a MOD-018 solo como colaborador. No es una contradiccion de fondo (ambas fichas coinciden en el contenido y en que la obligacion es RECOMENDADO y sin mecanismo ACE habilitado), pero para evitar listar la misma obligacion dos veces en la seccion 17.1 se adopto MOD-024 como modulo que sostiene la funcionalidad, por ser el propietario segun la jerarquia, y se menciono a MOD-018 unicamente como colaborador en la misma fila.
3. **Convencion de columnas de la seccion Q no es identica entre fichas.** La mayoria de las 26 fichas usa cuatro columnas booleanas "MUST HAVE | SHOULD HAVE | COULD HAVE | FUTURE"; MOD-007 usa la forma abreviada "MUST | SHOULD | COULD | FUTURE"; MOD-023 usa una unica columna de texto "Clasificacion" con los mismos cuatro valores en palabras. No afecta al contenido (los cuatro niveles son los mismos en todos los casos) pero exigio un tratamiento distinto al extraer la version de cada funcionalidad para las tablas de esta seccion; se resolvio leyendo cada formato de forma independiente y normalizando siempre a MVP/V1/V2/FUTURE segun la convencion de `02_validacion/propuesta_mapa_mvp.md`.
4. **(Actualizacion 2026-09-24, fase 3) Fila de OBL-PLAZO-04 (seccion PLAZO) omitia a su modulo propietario.** La columna "Modulo" citaba solo "MOD-008 (Documentos y Politicas); MOD-011 (ARCO-POL)", sin mencionar a MOD-012 (Portal del Titular), pese a que `06_mapa_definitivo_de_modulos.md` (seccion 8, tabla PLAZO) y `mapa_modulos.json` (obligaciones_propietarias de MOD-012) declaran a MOD-012 como propietario de OBL-PLAZO-04 y a MOD-011 como colaborador, igual que su obligacion gemela OBL-DOC-04 (mismo Art. 61 inc. 2). Se corrige anteponiendo MOD-012 en esa fila. De paso se unifica el tratamiento de version de ambas obligaciones gemelas (OBL-DOC-04 y OBL-PLAZO-04) a "MVP/V1", porque `03_modulos/MOD-012_ficha.md` (seccion Q) y la ficha resumida de MOD-012 en `06_mapa_definitivo_de_modulos.md` (seccion 3) documentan que ambas ya tienen cobertura parcial en el MVP por el formulario interno de MOD-011, completandose en V1 con el Portal del Titular; antes, una fila decia "V1" a secas y la otra "MVP" a secas para el mismo patron de cobertura.
5. **(Actualizacion 2026-09-24, fase 3) Fila de OBL-CONS-04 (seccion CONS) repetia el plazo de revocacion de OBL-CONS-03.** La funcionalidad de OBL-CONS-04 (Art. 26 inc. 4, consentimiento reforzado para datos sensibles) incluia la frase "Revocacion con flujo de dos plazos (5 + 5 dias habiles)", que corresponde solo a OBL-CONS-03 (Art. 30) segun `01_legal/matriz_obligaciones.json` (OBL-CONS-04 no tiene plazo y su articulo no regula revocacion). Se retira esa frase de la fila de OBL-CONS-04.

### Huecos detectados (piezas que ninguna ficha define de forma explicita con su propio OBL-ID)

6. **27 de las 100 obligaciones OBLIGATORIO/CONDICIONAL de la seccion 16 no tienen, en la seccion Q de su ficha propietaria, una fila cuya funcionalidad o justificacion cite su OBL-ID de forma literal.** Se resolvieron sin inventar ninguna funcionalidad nueva, atribuyendo cada una a la fila de Q ya existente que cubre la misma familia de campos, o a una funcionalidad descrita en las secciones D, E, G o I de la misma ficha (nunca de otra ficha distinta a la propietaria). La lista completa, por obligacion y ficha:
   - OBL-AMB-01 a 04 (MOD-004): la seccion Q no desglosa una fila por cada literal del Art. 3; se atribuyeron a "Cuestionario completo de 11 bloques" (AMB-01) y a "Deteccion de exclusiones del Art. 3 (seccion G.1)" (AMB-02 a 04); esa fila generica si esta en Q, pero sin desglosar los tres literales del Art. 3 por separado.
   - OBL-SENS-04 y OBL-SENS-08 (MOD-006): la fila de Q "Catalogo cerrado de categorias de datos sensibles" solo cita en su justificacion SENS-01 y SENS-06; SENS-04 (salud) y SENS-08 (videovigilancia) son subconjuntos del mismo catalogo, confirmados en la seccion A de la misma ficha, pero sin fila propia en Q.
   - OBL-TRAT-01 y OBL-TRAT-03 (MOD-006): no tienen fila propia en Q; se dedujeron de "Ficha de actividad de tratamiento" (campo finalidad, controla la desviacion de finalidad de TRAT-01) y de "Seleccion obligatoria de una base de licitud" (la fuente de acceso publico de TRAT-03 es una de esas seis bases).
   - OBL-SENS-07 (MOD-007): la fila de Q "Registro de consentimiento reforzado" cita en su justificacion "OBL-SENS-02/07" con una barra en vez de una coma, formato que la extraccion automatica no reconocio en la primera pasada; se corrigio manualmente.
   - OBL-DOC-01 (MOD-008): no tiene fila propia en Q; se confirmo en la seccion D (catalogo de tipos de documento, que incluye "Procedimiento ARCO-POL" citando expresamente OBL-DOC-01) que el motor documental generico de Q ya la cubre.
   - OBL-TRANSF-01 y OBL-TRANSF-02 (MOD-010): no tienen fila propia; se atribuyeron a "Registro manual basico de una transferencia", la unica fila de Q que cubre el nucleo del modulo.
   - OBL-ARCO-04, 05 y 06 (MOD-011, Cancelacion/Oposicion/Limitacion): no tienen fila propia en Q, pero el parrafo de "version minima vendible" de la misma seccion Q las menciona explicitamente como cubiertas por el mismo formulario interno seguro y el mismo plazo 20+20 que las demas ramas de ARCO-POL.
   - OBL-RET-01, 02 y 03 (MOD-016): no tienen fila propia; se atribuyeron a "Motor de retencion de datos del titular", la fila de Q que segun su propia justificacion "cita uno o mas OBL-RET".
   - OBL-PLAZO-01 y OBL-PLAZO-02 (MOD-023): la seccion Q de esta ficha usa el formato de columna unica "Clasificacion" (ver contradiccion 3) y sus dos primeras filas si citan estos OBL-ID de forma literal; el hueco aqui fue de extraccion automatica, no de contenido, y se corrigio leyendo la tabla manualmente.
   - OBL-SANC-02, 03, 04, 05, 06 y 09 (MOD-024): no tienen fila propia en Q; se atribuyeron a "Registro manual de un expediente de Procedimiento Sancionador con contadores de plazo", la fila que agrupa las nueve obligaciones SANC en la ficha.
   - OBL-PLAZO-05 (MOD-024): no tiene fila propia en Q con ese OBL-ID citado literalmente; se atribuyo a "Bandera regimen_reforma_659 con interruptor manual", confirmado por la seccion A de la misma ficha ("consulta pero no controla" esa bandera).
   - OBL-SANC-07 (MOD-024, RECOMENDADO, seccion 17): no aparece como fila propia en la seccion Q; se dedujo de las secciones D (campo `fecha_limite_prescripcion`), G (automatizacion del calculo) e I (alerta de prescripcion proxima) de la misma ficha.

7. **Ninguna ficha de modulo, ni el mapa de modulos, define una funcionalidad especifica para "detectar automaticamente que sucursal de una misma sociedad requiere un Aviso de Privacidad propio" mas alla de la pregunta de onboarding P-EMP-08 (MOD-004) y su fila RECOMENDADA correspondiente en la matriz de disparadores.** Esto no es una obligacion legal propia (ninguna norma exige avisos diferenciados por sucursal), por lo que no se incluyo en la seccion 16 ni en la 17; se deja constancia aqui porque el prompt del cliente (seccion 8.1) menciona sucursales como parte de la estructura de la empresa y ninguna ficha resuelve el caso de forma explicita mas alla de esa pregunta puntual del diagnostico.
8. **Ninguna ficha asigna un modulo propietario a la posibilidad de que la empresa opere bajo mas de un giro o sector a la vez** (por ejemplo, una empresa que es a la vez financiera y aseguradora, como el perfil "Grupo Financiero Itzalco" de `05_tipos_de_usuario.md`) para efectos de que el Diagnostico (MOD-004) le muestre las preguntas de mas de un sector simultaneamente. El diagnostico de MOD-004 precarga el sector desde el onboarding como una seleccion unica; el caso de un giro mixto queda sin una funcionalidad especifica que lo resuelva en ninguna de las 26 fichas revisadas. No se incluyo como funcionalidad propuesta de esta seccion porque excede el alcance de las secciones 16 a 18 (es un hallazgo de diseno del propio MOD-004); se anota aqui para que quede visible al equipo que redacte la seccion 24 (Preguntas pendientes) del blueprint final.

No se identifico ninguna funcionalidad exigida por el prompt del cliente (`00_prompt_analisis_funcional.md`, secciones 16 a 18) que ninguna ficha ni el mapa de modulos cubran ya de alguna forma; por eso esta seccion no incluye ninguna fila marcada "propuesta de esta seccion, no presente en las fichas".
