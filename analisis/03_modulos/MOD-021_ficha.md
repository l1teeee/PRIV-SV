...
| Estado | Seleccion unica | Obligatorio (autogenerado en "Pendiente" al crear) | Pendiente, En proceso, Bloqueada, En revision, Aprobada, Completada, No aplica | Transiciones segun la tabla de la seccion F | "En que paso del proceso esta la tarea." | Buena practica (Actualizacion 2026-09-24, fase 3: se retira "Vencida" del catalogo de este campo porque no es un valor excluyente del estado, ver el campo "Vencida (bandera)" y las secciones F.1 y F.2) |
| Vencida (bandera) | Booleano | Obligatorio (autogenerado en "No" al crear) | Si / No | Se activa automaticamente cuando la fecha limite se cumple sin que la tarea llegue a "Completada" o "No aplica"; no excluye ni reemplaza el valor de "Estado" (seccion F.1 y F.2) | "Indica si la tarea paso su fecha limite sin completarse, sin importar en que paso del proceso siga." | Buena practica (Actualizacion 2026-09-24, fase 3: campo nuevo, para alinear D.1 con el mecanismo ya descrito en F.1 y F.2) |
