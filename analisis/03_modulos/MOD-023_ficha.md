...
| Reclamo del titular ante la Direccion de Proteccion de Datos | 10 dh | Notificacion de la resolucion del delegado | MOD-011 | OBL-ARCO-14 | Art. 33 inc. 4 Lineamientos DPO | Dias habiles | Incertidumbre 1; ademas sin plazo propio fijado para la respuesta del delegado al requerimiento de informe de la ACE (matriz_obligaciones.json, campo `plazo.inicio` de OBL-ARCO-14) |
...
| Comunicacion del nombramiento del delegado a la ACE | 15 dh | Dia siguiente al nombramiento del delegado | MOD-002 | OBL-DPO-03 | Art. 10 Lineamientos DPO | Dias habiles (capa 5, autoridad) | Incertidumbre 6 |
...
| Cronometro de 72 horas de un incidente proximo a vencer (alertas intermedias) | El esquema real de umbrales lo define y ejecuta MOD-013 (ficha propietaria de OBL-INC-01 y OBL-INC-02, seccion I), no MOD-023: para la notificacion (OBL-INC-01), 50% del plazo consumido (36h, aviso temprano), 6 horas restantes (66h, critico) y vencimiento a las 72h; para el inicio de la revision exhaustiva (OBL-INC-02), 60 horas transcurridas | WARNING (aviso temprano y revision), CRITICAL (critico y vencido) | Responsable de Seguridad/IT, Delegado/Responsable interno, Administrador, Legal (segun el nivel, ver MOD-013 seccion I) | Plataforma + correo (+ SMS en el nivel critico) | Segun cada umbral (ver MOD-013 seccion I) | A Administrador si no hay accion en 6 horas (aviso temprano); de inmediato a Gerencia en el nivel critico; a Delegado a las 70 horas si sigue sin marcarse el inicio de la revision | El calculo se cierra (incidente notificado, o inicio de revision marcado) o alcanza las 72 horas y pasa a la bandera Vencido |
...
Los umbrales de 24, 48 y 60 horas que `01_legal/sweep_plazos_calendario_retencion.md` (seccion 2.5) proponia como valor por defecto para el cronometro de incidentes no fueron los que finalmente adopto el modulo propietario. MOD-013_ficha.md (seccion I) define en su lugar el esquema descrito en la fila anterior (50% del plazo, 6 horas restantes y vencimiento a las 72h para OBL-INC-01; 60 horas para el inicio de la revision de OBL-INC-02); MOD-023 solo calcula las fechas limite y las horas transcurridas, MOD-013 es quien define y ejecuta el esquema real de alertas sobre esos calculos.
